...
train_path_pos1 = fullfile(data_path, 'att_faces'); %Positive training examples. 36x36 head crops
...
features_pos = get_positive_features( train_path_pos, train_path_pos1, feature_params );
...
% % % confidence level.
% % features_hard_neg = hard_neg_mining(non_face_scn_path, w, b, feature_params);
% % X = [features_pos; features_neg; features_hard_neg]';
% % Y = [ones(size(features_pos, 1), 1); -ones(size(features_neg, 1), 1); -ones(size(features_hard_neg, 1), 1)]';
% % 
% % [w, b] = vl_svmtrain(X, Y, 0.0001); 
